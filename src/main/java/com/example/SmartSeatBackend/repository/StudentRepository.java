...
public interface StudentRepository extends JpaRepository<Students, String> {
    ...
    @Query(value =
            "SELECT st.enrollment_no AS enrollmentNo, st.college_id AS collegeId FROM students st " +
                    ...
                    "WHERE ss.subject_code = :subCode AND st.branch = :branch AND st.semester = :sem " +
                    "UNION " +
                    "SELECT st.enrollment_no AS enrollmentNo, st.college_id AS collegeId FROM students st " +
                    "LEFT JOIN back_subjects bs ON st.enrollment_no = bs.enrollment_no " +
                    "WHERE bs.backlog_subjects = :subCode AND st.branch = :branch AND st.semester > :sem",
            nativeQuery = true)
    List<StudentEnrollmentDTO> findStudentsForExam(
